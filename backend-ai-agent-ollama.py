...
class SubtaskList(BaseModel):
    ...
    subtasks: List[SubtaskSuggestion] = Field(
        description="List of 3-7 subtasks, ordered logically"
    )
    reasoning: str = Field(
...
Classify this task.""")
            ...
            logger.info(f"Task classified: {result.get('category')} (priority: {result.get('priority')})")
            return result
    ...
    def suggest_subtasks(self, title: str, description: str) -> dict:
        ...
        """
        try:
            parser = JsonOutputParser(pydantic_object=SubtaskList)

            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a task breakdown assistant.
                Break down the given task into 3-7 concrete, actionable subtasks.
